...
def get_all_change_files(debug=False):
    """Get all change files from all changelog directories."""
    script_dir = Path(__file__).parent

    # Define all changelog directories to check
    changelog_dirs = [
        script_dir / "changes",
        script_dir.parent / "components" / "app" / "changelog" / "changes",
        script_dir.parent / "components" / "tenant-manager" / "changelog" / "changes",
    ]

    # Get all .md files and extract their base names (without .md extension)
    change_files = set()
    for changes_dir in changelog_dirs:
        if changes_dir.exists():
            md_files = list(changes_dir.glob("*.md"))
            if debug:
                print(f"Found {len(md_files)} files in {changes_dir}", file=sys.stderr)
            for file_path in md_files:
                # Skip dummy files and other non-changelog files
                if file_path.name not in [".dummy", "README.md"]:
                    change_files.add(file_path.stem)
                elif debug:
                    print(f"  Skipped {file_path.name}", file=sys.stderr)
        elif debug:
            print(f"Directory does not exist: {changes_dir}", file=sys.stderr)
...
def get_unused_changes(debug=False):
    """Get all change IDs that haven't been used in any release."""
    all_changes = get_all_change_files(debug=debug)
...
def create_release_file(version, title, description, debug=False):
    ...
    # Get unused changes
    unused_changes = get_unused_changes(debug=debug)
    ...
    try:
        ...
        # Show summary of where changes were found
        all_changes = get_all_change_files(debug=debug)
        used_changes = get_used_changes()
        print(f"\nSummary:")
        print(f"  Total change files found: {len(all_changes)}")
        print(f"  Previously used changes: {len(used_changes)}")
        print(f"  Changes included in this release: {len(sorted_changes)}")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s v5.3.0 --title "Tenzir Platform v5.3.0" --description "This release includes bug fixes and new features."
  %(prog)s v5.3.1 --title "Tenzir Platform v5.3.1" --description ""
        """
    )
    ...
    parser.add_argument("--debug", action="store_true",
                       help="Show detailed information about directory scanning")
    ...
    # Create the release
    create_release_file(args.version.strip(), args.title.strip(), args.description, debug=args.debug)
