...
# Null resource to create ca.pem file in EFS using the Terraform CA certificate
resource "null_resource" "create_ca_file_in_efs" {
  depends_on = [
  ...
  provisioner "local-exec" {
    command = <<-EOT
      # Create a temporary EC2 instance to mount EFS and create the ca.pem file
      INSTANCE_ID=$(env AWS_PROFILE=tenzir-playground aws ec2 run-instances \
        --image-id ami-0c94855ba95b798c7 \
        --instance-type t3.micro \
        --security-group-ids ${aws_security_group.efs.id} \
        --subnet-id ${aws_subnet.platform.id} \
        --user-data "#!/bin/bash
yum update -y
yum install -y amazon-efs-utils
mkdir -p /mnt/efs
mount -t efs ${aws_efs_file_system.ca_storage.id}:/ /mnt/efs
cat << 'EOF' > /mnt/efs/ca.pem
${tls_self_signed_cert.ca.cert_pem}
EOF
chmod 644 /mnt/efs/ca.pem
umount /mnt/efs
shutdown -h now" \
      ...
      env AWS_PROFILE=tenzir-playground aws ec2 wait instance-stopped --instance-ids $INSTANCE_ID
      
      # Terminate the temporary instance
      env AWS_PROFILE=tenzir-playground aws ec2 terminate-instances --instance-ids $INSTANCE_ID
    EOT
  }

  triggers = {
    ca_cert_content = sha256(tls_self_signed_cert.ca.cert_pem)
  }
}